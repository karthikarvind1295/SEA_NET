
#from utils import extract_labels
import os
import tensorflow as tf
import glob
import cv2
import json
import numpy as np

def extract_labels(label_path):
    

    with open(label_path,'r') as file:
        data = json.load(file)

    label = data['shapes']
    points = []
    for i in range(len(label)):
        points.append(np.array(label[i]['points']))
    #print(points)

    return points

def numerical_sort(img_path):
    img_num = img_path.split("_")[-1]
    frame_number = int(img_num.split(".")[0])
    return frame_number

def indices_extraction(indice_location, name):
    json_file = os.path.join(indice_location, name,'frame.json')
    with open(json_file,'r') as file:
        data = json.load(file)

    return np.array(data[0]['indices'])

def Datasets(image_path, demo_names):

    all_img_path = []
    all_locations = []
    all_labels = []
    for name in demo_names:
        indices = indices_extraction(image_path, name)
        path = os.path.join(image_path, name)
        for img_path in sorted(glob.glob(path + "/*.png"), key= numerical_sort):
            img_num = img_path.split("_")[-1]
            frame_number = int(img_num.split(".")[0])
            label_path = os.path.join(path, 'label', 'image_%d.json'%frame_number)

            location = extract_labels(label_path)

            if frame_number <= indices[0]:
                label = 0
            elif frame_number > indices[0] and frame_number <= indices[1]:
                label = 1
            elif frame_number > indices[1] and frame_number <= indices[2]:
                label = 2
            else:
                label = 3
            
            all_img_path.append(img_path)
            all_locations.append(location)
            all_labels.append(np.round(label))
    
    return all_img_path, all_locations, all_labels
    

class CustomDataset_Keras_Test(tf.keras.utils.Sequence):

    def __init__(self, image_path, demo_names):
        """
        The dataloader accepts frames of each demonstration and links with its corresponding label location.
        :param image_path : Location of the image files
        :param label_path : Location of the location json file
        :param indices_path : Location of the json file of the frame of the segmented images
        :param demo_names : The list of file names inside image_path
        """
        self.image_path = image_path
        self.demo_names = demo_names


        self.x, self.y, self.z = Datasets(self.image_path, self.demo_names)
        
        
        

    
    def __len__(self):
        return len(self.x)

    def __getitem__(self, idx):

#         batch_x = self.x[idx * self.batch_size: (idx + 1) *self.batch_size]
#         batch_y = self.y[idx * self.batch_size:(idx + 1) * self.batch_size]

        img_path = self.x[idx]
        goal_location = self.y[idx]
        label = self.z[idx]
        # img = cv2.imread(img_path)
        # img = torch.from_numpy(img)

        
        # sample = self.transform(img)

        # #Supress this for getting only the points as the target
        # label = np.zeros(sample.shape[1:])
        # label[int(goal_location[0,0]), int(goal_location[0,1])] = 1
        # #label = label.reshape(1, np.prod(label.shape))
        # label = torch.from_numpy(label)


        return np.array(cv2.imread(img_path)), np.array(goal_location), np.array(label)
